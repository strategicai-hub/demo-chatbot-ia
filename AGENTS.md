...
>
> **Propagação em cascata.** O `plano-pleno-template` é derivado deste template e recebe commits via cherry-pick aqui. Os **clientes do plano-pleno** ficam no AGENTS.md do próprio `plano-pleno-template` (sync em cascata: `start → pleno → clientes do pleno`).
...
O script:
1. Lê a lista de repos derivados desta seção do AGENTS.md
